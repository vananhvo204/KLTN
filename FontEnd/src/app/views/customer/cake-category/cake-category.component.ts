...
export class CakeCategoryComponent implements OnInit {
  ...
  onSliderChange(selectedValues: number[]) {
    ...
    console.log(this.cakeFilter.price1)
    this.price1 = selectedValues[0];
    this.price2 = selectedValues[1];
    
  }
filterByPrice(){
  this.cakeFilter.price1 = this.price1
  this.cakeFilter.price2 = this.price2
  this.filter();
}
  ...
  ngOnInit() {
   ...
   // this.initialCakeFilter();
    this.cakeFilter.nameCake = this.searchBy;
    this.filter();
  }
  initialCakeFilter() {
    this.cakeFilter = ({
  ...
  refreshCakeList() {
    this.cakeFilter.category_id = null;
    this.cakeFilter.price1 = null;
    this.cakeFilter.price2 = null;
    this.cakeService.getCakeList().subscribe((res) => {
      this.cakes = res as Cake[];
  ...
  gettypeCategory(category_id) {
    if (this.cakeFilter.category_id == category_id) {
      this.cakeFilter.category_id = null;
    } else {
      this.cakeFilter.category_id = category_id;
    }
    this.filter();
  }
  filter() {
    this.cakeService.filterCake(this.cakeFilter).subscribe(res => {
      this.cakes = res as Cake[]
    })
  }
  ...
  searchNameCake() {
    ...
    this.cakeFilter.nameCake = this.searchForm.value.nameCake;
    this.filter();
  }
  onEditClick(event){
    this.cakeFilter.sortByPrice = event;
    this.filter();
  }
  // sort() {
  //   let selected = $('select[name=orderby] option').filter(':selected').val()
  //   this.cakeFilter.sortByPrice = selected;
  //   this.filter();
  // }
